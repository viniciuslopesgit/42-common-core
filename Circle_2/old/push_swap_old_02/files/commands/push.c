...
/*   Updated: 2025/02/24 00:41:52 by viniciuslop      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/push_swap.h"

static void	push(node **dst, node **src) //Define a function that pushes a top node, from one stack to another's top node
{
	node	*push_node; //Used to store the pointer to the node to be pushed

	if (!*src) //The top node of a stack to be pushed
		return ;
	push_node = *src; //The top node to push is assigned to the `node` variable
...
void	pa(node **a, node **b, bool print) //Push on top of `b`, the top `a` and print the instruction
{
	push(a, b); 
	if (!print) 
		ft_printf("pa\n");
...
void	pb(node **b, node **a, bool print) //Push on top of `a`, the top `b` and print the instruction
{
	push(b, a);
	if (!print)
		ft_printf("pb\n");
